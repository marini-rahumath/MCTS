...
var PLib = 
	{
		...
		f: 3.35281066474748E-3, s: 1.012229, ge: 3.986008E5,
		qoms2t: 1.880279E-09,
		secday: 8.6400E4,
		...
		FindAOS: function()
		{
			...
			if (PLib.AosHappens(indx) && PLib.Geostationary(indx) == 0 && PLib.Decayed(indx, PLib.daynum) == 0)
			{
				...
				{
					if (Math.abs(PLib.sat_ele) < 0.005)
		...
		// Period to predict over
		// Default 1 day/24hours/1440min/86400sec/86400000millisec
		addDay: function(dt,searchPeriod)
		{
		    var d = new Date(dt.getTime() + 86400000*searchPeriod);
		    return d;
		},

		getTodaysPasses: function(searchPeriod)
		{
			...
			for (z = 0; z < PLib.sat.length; z++)
			{
				...
				if ((start >= now - 31557600) && (start <= now + 31557600))
				{
					...
					if (PLib.AosHappens(indx) && PLib.Geostationary(indx) == 0 && PLib.Decayed(indx, PLib.daynum) == 0)
					{
						...
						while (PLib.Daynum2Date(PLib.daynum) < PLib.addDay(d,searchPeriod))
						{
							var satInfo = new Object();
							...
							satInfo.riseRange
								...
								= PLib.irk;
							satInfo.orbitNumber = PLib.rv;
							satInfo.coordinatesAzi = [];
							satInfo.coordinatesEle = [];
							...
							while (PLib.iel >= 0)
							{
								...
								lastel = PLib.iel;
								PLib.daynum += Math.cos((PLib.sat_ele - 1.0) * PLib.deg2rad) * Math.sqrt(PLib.sat_alt) / 502500.0;
								PLib.Calc();
								satInfo.coordinatesAzi.push(PLib.sat_azi);
								satInfo.coordinatesEle.push(PLib.sat_ele);
							}
		...
		formatVisibility: function(data)
		{
			// V = Visible, D = Daylight, E = Eclipsed, U = Unknown
			if(data == "+"){
				return "Visible";
			}else if(data == "*"){
				return "Daylight";
		...
		getNextPass: function()
		{
			indx = 0;
			PLib.daynum = PLib.CurrentDaynum();
			var AOSFound = PLib.FindAOS();
			var timeAOS = PLib.Daynum2Date(AOSFound);
			var timeLOS = PLib.Daynum2Date(PLib.FindLOS());
			
			PLib.PreCalc(indx);
			PLib.daynum = AOSFound;
			var coordinatesAzi = [];
			var coordinatesEle = [];
			while (PLib.iel >= 0)
			{
				PLib.daynum += Math.cos((PLib.sat_ele - 1.0) * PLib.deg2rad) * Math.sqrt(PLib.sat_alt) / 502500.0;
				PLib.Calc();
				coordinatesAzi.push(PLib.sat_azi);
				coordinatesEle.push(PLib.sat_ele);
			}
			var nextPass = {aos:timeAOS,los:timeLOS, cazi: coordinatesAzi, cele: coordinatesEle};			
			return nextPass;
		},

		liveTrack: function()
		{
			// Set the satellite from tleData
			indx = 0;
			PLib.PreCalc(indx);
			PLib.CalcLive();
			var liveData = {
				// tle: PLib.sat[indx],
				// tlel1: PLib.sat[indx].line1,
				// tlel2: PLib.sat[indx].line2,
				name: PLib.tle.sat_name,
				azi: PLib.sat_azi,
				ele: PLib.sat_ele,
				range: PLib.sat_range,
				rrate: PLib.sat_range_rate,
				ssp: PLib.ssp,
				lat: PLib.sat_lat,
				long: PLib.sat_lon,
				fpkm: PLib.fk,
				...
				alt: PLib.sat_alt,
				vel: PLib.sat_vel,
				dfactor: PLib.dop_factor,
				dshift: PLib.dop_shift_100m,
				signl: PLib.signal_loss,
				signd: PLib.signal_delay,
				ma256: PLib.ma256,
				orbphase: PLib.orb_phase,
				orbnum: PLib.rv,
				orbperiod: PLib.orb_period,
				visible: PLib.formatVisibility(PLib.findsun)
			}
			// console.log(liveData);
			return liveData;
		},

		simulateTrack: function()
		{
			// Set the satellite from tleData
			indx = 0;
			PLib.PreCalc(indx);
		...
		getSimNextPass: function(simtime)
		{
			indx = 0;
			PLib.daynum = simtime;
			var AOSFound = PLib.FindAOS();
		...
		longlat2locator: function(longitude, latitude, pair_count)
		{
		    var x_or_y, pair, locvalue, divisions; // int
		    var square_size, ordinate; // double
		    var loc_char_range = [18, 10, 24, 10, 24, 10];
		    var locator = [0,0,0,0,0,0];
		    ...
		    for (x_or_y = 0;  x_or_y < 2;  ++x_or_y) {
		        ...
		        for (pair = 0;  pair < pair_count;  ++pair) {
		            ...
		            locvalue = Math.floor(ordinate / square_size);
		            ordinate -= square_size * locvalue;
		            locvalue = String.fromCharCode(locvalue + ((loc_char_range[pair] == 10) ? '0':'A').charCodeAt(0));
		            locator[pair * 2 + x_or_y] = locvalue;
		        }
		    }
		    return locator.toString().replace(new RegExp(',','g'),'');
		},

		// Copy of Calc: function() with altered parts
		CalcLive: function()
		{
			var zero_vector = new PLib.vector_t();
			var vel = new PLib.vector_t();
			...
			PLib.jul_utc = PLib.CurrentDaynum() + 2444238.5;
			...
			// SSP Location
			PLib.ssp = PLib.longlat2locator(PLib.sat_lon, PLib.sat_lat, 3);
			
			// Doppler
			// speed of light = c = 299792.4580
			PLib.dop_factor = (1 + PLib.sat_range_rate / 299792.4580);
			PLib.dop_shift_100m = -100.0e06 * (PLib.sat_range_rate / 299792.4580);

			// Signal
			PLib.signal_loss = 72.4 + 20.0 * Math.log(PLib.sat_range) / Math.LN10;
			PLib.signal_delay = PLib.sat_range / 299792.4580 * 1000;

			// Footprint, fk = in km, fm = in miles
			PLib.fk = 12756.33 * Math.acos(PLib.xkmper / (PLib.xkmper + PLib.sat_alt));
			PLib.fm = PLib.fk / 1.609344;
			
			// Orbit period (min)
			PLib.orb_period = ((2 * Math.PI) * (PLib.sat_alt + PLib.xkmper)) * (Math.sqrt((PLib.sat_alt + PLib.xkmper)/PLib.ge)) / 60;
			// Orbit number
			...
			// Orbit phase
			PLib.orb_phase = 360.0 * (PLib.phase / PLib.twopi);
			// Mean anomaly
		...
		CalcPos: function(date)
		{
			...
			PLib.jul_utc = PLib.Date2DayNum(date) + 2444238.5;
			...
			// SSP Location
			//PLib.ssp = PLib.longlat2locator(PLib.sat_lon, PLib.sat_lat, 3);
            return {lat: PLib.sat_lat, lng: PLib.sat_lon}
...
// 		[
// "GALASSIA",
// "1 41170U 15077E   16270.69050786  .00003670  00000-0  17369-3 0  9995",
// "2 41170  14.9892 290.8753 0013734 340.8085  19.1717 15.11036744 43148"
// 		]
// 		,
// 		[
// "KENT RIDGE 1",
// "1 41167U 15077B   16270.41102530  .00001279  00000-0  38990-4 0  9993",
// "2 41167  14.9855 298.9488 0011240 314.9183 163.1829 15.08488775 43042"
// 		]
